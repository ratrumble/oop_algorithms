...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        PriorityQueue<NodeType> visited = new PriorityQueue<NodeType>();
        ...
        while(!pq.isEmpty()) {
          ...
          if(!visited.contains(currentPath.end.data)) {
            
            visited.add(currentPath.end.data);
            ...
            for(int i = 0; i < currentPath.end.edgesLeaving.size(); i ++) {
              ...
              pq.add(new Path(currentPath, currentPath.end.edgesLeaving.get(i)));
        ...
        throw new NoSuchElementException(); // could not access end node
    }
